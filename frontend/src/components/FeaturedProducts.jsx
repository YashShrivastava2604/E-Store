...
const FeaturedProducts = ({ featuredProducts }) => {
  ...
  // Normalize incoming prop to always be an array
  const products = useMemo(() => {
    if (!featuredProducts) return [];
    if (Array.isArray(featuredProducts)) return featuredProducts;
    if (Array.isArray(featuredProducts.data)) return featuredProducts.data;
    if (Array.isArray(featuredProducts.products)) return featuredProducts.products;
    return [];
  }, [featuredProducts]);
  ...
  // Keep currentIndex valid when products length or itemsPerPage change
  useEffect(() => {
    const maxStart = Math.max(0, products.length - itemsPerPage);
    if (currentIndex > maxStart) {
      setCurrentIndex(maxStart);
    }
  }, [products.length, itemsPerPage, currentIndex]);

  const nextSlide = () => {
    setCurrentIndex((prevIndex) =>
      Math.min(prevIndex + itemsPerPage, Math.max(0, products.length - itemsPerPage))
    );
  };
  ...
  const isEndDisabled = currentIndex >= Math.max(0, products.length - itemsPerPage);
            ...
            >
              {products.map((product) => (
                <div key={product._id} className="w-full sm:w-1/2 lg:w-1/3 xl:w-1/4 flex-shrink-0 px-2">
                  <div className="bg-emerald-800 bg-opacity-10 backdrop-blur-sm rounded-lg shadow-lg overflow-hidden h-full transition-all duration-300 hover:shadow-xl border border-emerald-500/30">
                    <div className="overflow-hidden">
                      <img
                        src={product.image}
                        alt={product.name}
